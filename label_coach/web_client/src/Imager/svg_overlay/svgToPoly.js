    ...
    makeAbsolute(parseSVG(svgPathString))
        .forEach(cmd => {
            switch (cmd.code) {
                ...
                case 'C':
                    sampleCubicBezier(cmd.x0, cmd.y0, cmd.x1, cmd.y1, cmd.x2, cmd.y2, cmd.x, cmd.y);
                    add(cmd.x, cmd.y);
                    break;
                ...
                case 'S':
                    ...
                    sampleCubicBezier(cmd.x0, cmd.y0, x1, y1, cmd.x2, cmd.y2, cmd.x, cmd.y);
                    add(cmd.x, cmd.y);
                    break;

                default:
                    console.error('Our deepest apologies, but ' + cmd.command + ' commands (' + cmd.code +
...
${polys.map(poly => `  <${poly.closed ? 'polygon' : 'polyline'} points="${poly.join(' ')}"/>`)
       .join("\n")}
</g>
</svg>
	`.trim());
